...
//    }
    ...
    def temporalSpanText = { attrs ->
        if (attrs.start && attrs.end) {
            out << "${g.message(code: "change.js.transformer01")} ${attrs.start}  ${g.message(code: "change.js.transformer02")}  ${attrs.end}."
        } else if (attrs.start) {
            out << "${g.message(code: "change.js.transformer01")} ${attrs.start} ${g.message(code: "change.js.transformer01")}"
        } else if (attrs.end) {
            out << "${g.message(code: "change.js.transformer05")} ${attrs.end}."
        } else if (attrs.change) {
            out << "${g.message(code: "change.js.transformer06")}"
        }
    }
    ...
    def stateCoverage = {attrs ->
        ...
        if (attrs.states.toLowerCase() in ["all", "all states", "australian states"]) {
            out << g.message(code: "change.js.transformer11")
        } else {
            out << g.message(code: "change.js.transformer11") + attrs.states.encodeAsHTML() + "."
        }
    }
        ...
        switch (attrs.home) {
            case 'dataSets':
                hereLink = attrs.atBase == 'true' ?
                        "Datasets" :
                        link(controller:'public', action:'dataSets') {g.message(code: 'breadcrumb.01', args: [])}
                break
            case 'dashboard':
                hereLink = attrs.atBase == 'true' ?
                        "Dashboard" :
                        link(controller:'dashboard', action:'index') {g.message(code: 'breadcrumb.02', args: [])}
                break
            default:
                hereLink = attrs.atBase == 'true' ?
                        "Collections" :
                        link(controller:'public', action:'map') {g.message(code: 'breadcrumb.03', args: [])}
        }
        if (grailsApplication.config.skin.includeBaseUrl) {
            out << "<a href='${grailsApplication.config.ala.baseURL}'>" + g.message(code: 'breadcrumb.04', args: []) + "</a> <span class=\"icon icon-arrow-right\"></span> "
        }
        out <<   hereLink
    }
    ...
    def displayLicenseType = {attrs ->
        ...
        if (license) {
            ...
            if (attrs.imageOnly && image) {
                ...
                out << "${display.display?: g.message(code: 'taglib.nolicence')} ${attrs.version?:''} (${license}) ${imageHtml}"
            }
            else {
                out << "${display.display?: g.message(code: 'taglib.nolicence')} ${attrs.version?:''} (${license})"
            }
        }
    }
    ...
    def lastChecked = { attrs ->
        if (attrs.date) {
            out << """<span id="updated">${message(code: 'taglib.laschecked', default: 'This resource was last checked for updated data on')}
    ...
    def dataCurrency = { attrs ->
        if (attrs.date) {
            out << """<span id="currency">${message(code: 'taglib.dataCurrency', default: 'The most recent data was published on')}
                <b>${new SimpleDateFormat("dd MMM yyyy").format(attrs.date)}</b>.</span>"""
        }
    }
